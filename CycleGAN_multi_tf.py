...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=3e-4, image_shape=(192, 192, 2),
                 date_time_string_addition='', image_folder='', mode='train'):
        self.img_shape = image_shape
        self.channels = self.img_shape[-1]
        self.normalization = tfa.layers.InstanceNormalization
        # Hyper parameters
        self.lambda_1 = 5.0  # Cyclic loss weight A_2_B
        self.lambda_2 = 5.0  # Cyclic loss weight B_2_A
        self.lambda_D = 5.0  # Weight for loss from discriminator guess on synthetic images
...
#######################################################################################################
        ...
        nr_T1_train_imgs = None
        nr_T2_train_imgs = None

        nr_S1_test_imgs = None
        nr_S2_test_imgs = None
        nr_T1_test_imgs = None
        nr_T2_test_imgs = None
        ...
        data = load_data.load_data(nr_of_channels=1,
                                   ...
                                   nr_T1_train_imgs=nr_T1_train_imgs,
                                   nr_T2_train_imgs=nr_T2_train_imgs,
                                   nr_S1_test_imgs=nr_S1_test_imgs,
                                   nr_S2_test_imgs=nr_S2_test_imgs,
                                   nr_T1_test_imgs=nr_T1_test_imgs,
                                   nr_T2_test_imgs=nr_T2_test_imgs,
                                   subfolder=image_folder)

        self.S1_train = data["trainS1_images"]
        self.S2_train = data["trainS2_images"]
        self.T1_train = data["trainT1_images"]
        self.T2_train = data["trainT2_images"]
        self.S1_test = data["testS1_images"]
        self.S2_test = data["testS2_images"]
        self.T1_test = data["testT1_images"]
        self.T2_test = data["testT2_images"]
        self.testS1_image_names = data["testS1_image_names"]
        self.testS2_image_names = data["testS2_image_names"]
        self.testT1_image_names = data["testT1_image_names"]
        self.testT2_image_names = data["testT2_image_names"]

        self.S_train = np.concatenate((self.S1_train, self.S2_train), axis=-1)
        self.T_train = np.concatenate((self.T1_train, self.T2_train), axis=-1)
        self.S_test = np.concatenate((self.S1_test, self.S2_test), axis=-1)
        self.T_test = np.concatenate((self.T1_test, self.T2_test), axis=-1)
...
# Save and load
    ...
    def load_model_and_generate_synthetic_images(self):
        ...
        def save_image(image, name, domain):
            ...
            np.save(os.path.join('/home/timo/ms_data_bran_03_2020/generated_images/npy_data', domain, name), image)
        ...
        for i in range(len(synthetic_images_T)):
            # Get the name from the image it was conditioned on
            name = self.testS1_image_names[i].strip('.npy') + '_synthetic.npy'
            synt_T1 = scale_image_to_value_range(synthetic_images_T[i, :, :, 0])
            synt_T2 = scale_image_to_value_range(synthetic_images_T[i, :, :, 1])
            save_image(synt_T1, name, 'T1')
            save_image(synt_T2, name, 'T2')

        print('{} synthetic images for each domain have been generated and placed in ./generate_images/synthetic_images'
