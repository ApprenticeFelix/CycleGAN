...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=3e-4, image_shape=(320, 320, 3),
...
# Save and load
    ...
    def load_model_and_generate_synthetic_images(self):
        ...
        for iid in test_id:
            ...
            t2_ch1 = out[0, ...]
            t2_ch1 = self.denoise_norl(t2_ch1)
            t2_ch2 = out[1, ...]
            t2_ch2 = self.denoise_norl(t2_ch2)
            t2_ch3 = out[2, ...]
            t2_ch3 = self.denoise_norl(t2_ch3)
            #print(np.shape(t2_ch1))

            test_T_3ch = np.concatenate((t2_ch1[..., np.newaxis], t2_ch2[..., np.newaxis], t2_ch3[..., np.newaxis]), axis=3)
            pred_3D = []
            for ss in range(np.shape(test_T_3ch)[0]):

                print(ss)
                pred = extract_patches(self.G_A2B, test_T_3ch[ss], intl=160)
                pred_ = pred[..., 0]
                pred_ = (pred_ + 1)*127.5
            ...
            sitk.WriteImage(pred_3D_orig_i, str(iid)+'.nii.gz')
